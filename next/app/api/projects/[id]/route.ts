...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const project = await projectStore.findById(params.id)
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const payload = await verifyToken(token, process.env.JWT_SECRET!)
    const existing = await projectStore.findById(params.id)
    if (!existing || existing.userId !== payload.sub) return new NextResponse('Not Found', { status: 404 })
    const body = await req.json()
    const updated = await projectStore.update(params.id, { name: body.name, instructions: body.instructions })
